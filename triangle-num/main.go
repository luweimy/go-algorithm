...
// - cache: 用于存储每个位置的最大路径和，默认为-1
func TriangleMaxSum2(ns, cache [][]int, i, j int) int {
	if len(ns) == i {
		return 0
	}
	if cache[i][j] != -1 {
	...
	r := ns[i][j] + maxInt(TriangleMaxSum2(ns, cache, i+1, j), TriangleMaxSum2(ns, cache, i+1, j+1))
	cache[i][j] = r
	return r
}
...
func TriangleMaxSum3(ns [][]int) int {
	...
	var cache = ns[len(ns)-1]
	for i := len(ns) - 2; i >= 0; i-- {
		// 计算当前行每一个位置的最大路径和
		for j := 0; j < len(ns[i]); j++ {
			// 当前行的元素ns[i][j]与下面那一行的左右两个位置中最大的那个相加就是当前最大路径和
			// cache[j], cache[j+1] 就是缓存的下面那行左右的两个位置的最大路径和
			// 下面那行的cache[j]会被当前行位置j的最大路径和覆盖，但是不影响
			cache[j] = ns[i][j] + maxInt(cache[j], cache[j+1])
		}
	}
	return cache[0]
}
...
func main() {
	...
	cache := makeEmptyCache(ns)
	fmt.Println("TriangleMaxSum2=>", TriangleMaxSum2(ns, cache, 0, 0))
	fmt.Println("TriangleMaxSum2=> cache", cache)
...
func makeEmptyCache(ns [][]int) [][]int {
	var cache = make([][]int, len(ns))
	for i, n := range ns {
		cache[i] = makeEmptySlice(n)
	}
	return cache
}
